...
const orderController = {
  ...
  createOrder: asyncHandler(async (req, res) => {
    const { shippingAddressId, paymentMethod, notes, totalAmountFrontend } =
      req.body;
    ...
    if (!totalAmountFrontend || totalAmountFrontend <= 0) {
      return res
        .status(400)
        .json({ success: false, message: "Invalid total amount provided." });
    }
      ...
      populate: [
        {
          path: "product",
          model: "Product",
          select: "title images category unit",
        },
        {
          path: "farmer",
    ...
    try {
      ...
      await order.save();
      console.log(
        "Order created with OTP reference:",
        order.metadata.otpReference,
      );
      ...
      if (cart) {
        ...
        await cart
          .save()
          .catch((err) => console.error("Cart clearance warning:", err));
      }
      ...
      if (otpRecord && otpRecord._id) {
        await OTPVerification.findByIdAndDelete(otpRecord._id)
          .then(() =>
            console.log("OTP record cleaned up due to order creation failure"),
          )
          .catch((cleanupErr) =>
            console.error("OTP cleanup failed:", cleanupErr),
          );
      }
  ...
  verifyOrderOTP: asyncHandler(async (req, res) => {
    ...
    console.log(
      "Order found for OTP verification, OTP Reference:",
      order.metadata?.otpReference,
    );
    ...
    try {
      ...
      const finalOrderStatus =
        order.paymentMethod === "cod" ? "confirmed" : "confirmed";
      const finalPaymentStatus =
        order.paymentMethod === "cod" ? "pending" : "completed";
      ...
      if (!updatedOrder) {
        ...
        return res
          .status(400)
          .json({ success: false, message: "Order not found for this OTP" });
      }
  ...
  resendOrderOTP: asyncHandler(async (req, res) => {
    ...
    console.log(
      "Resend OTP requested for Order ID:",
      orderId,
      "by user:",
      userEmail,
    );
    ...
    console.log(
      "Order found for OTP resend, current OTP reference:",
      order.metadata?.otpReference,
    );
    ...
    try {
      ...
      await OTPVerification.findByIdAndDelete(newOTPRecord._id).catch(
        (cleanupErr) =>
          console.error(
            "Failed to cleanup OTP record after email failure:",
            cleanupErr,
          ),
      );
  ...
  updateOrderStatus: asyncHandler(async (req, res, next) => {
    ...
    try {
      ...
      console.error(
        " Order status email failed, but status was updated:",
        emailError,
      );
      // Continue even if email fails
    }
  ...
  cancelOrder: asyncHandler(async (req, res, next) => {
    ...
    try {
      ...
      console.error(
        "Cancellation email failed, but order was cancelled:",
        emailError,
      );
      // Continue even if email fails
    }
  ...
  resendOrderOTP: asyncHandler(async (req, res) => {
    ...
    if (
      ...
      return res
        ...
        .json({
          success: false,
          message: "OTP cannot be resent for this order",
        });
    }
